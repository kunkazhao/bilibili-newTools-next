...
describe("BlueLinkMapPageView", () => {
  ...
  it("shows commission label and remark line", () => {
    const entry = {
      id: "e1",
      account_id: "a1",
      category_id: "c1",
    ...
    expect(screen.getAllByText(/佣金/).length).toBeGreaterThan(0)
    expect(screen.getAllByText(/备注/).length).toBeGreaterThan(0)
    const header = screen.getByTestId("blue-link-card-header")
    const metrics = screen.getByTestId("blue-link-card-metrics")
    expect(header.className).toContain("items-start")
    expect(metrics.className).toContain("flex-nowrap")
  })

  it("renders toolbar texts in Chinese", () => {
    render(
      <BlueLinkMapPageView
        {...baseProps}
        loading={false}
    ...
    expect(screen.getByPlaceholderText("搜索商品名称")).not.toBeNull()
    expect(screen.getAllByText("导入蓝链").length).toBeGreaterThan(0)
  })
})
