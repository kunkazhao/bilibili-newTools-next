    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <PrimaryButton onClick={() => showToast("一键加购功能待迁移", "info")}>
              开始加购
            </PrimaryButton>
            <PrimaryButton onClick={() => showToast("清空功能待迁移", "info")}>
              清空列表
            </PrimaryButton>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
        <div className="space-y-3">
          <label className="text-sm font-medium text-slate-700">商品链接</label>
          <Input placeholder="每行一个商品链接，支持批量粘贴" />
